  ...
  return (
    <PageLayout title={"Add Subcategory"}>
      ...
      <Box sx={{ p: 2, bgcolor: "#f5f5f5", mb: 2, borderRadius: 1 }}>
        ...
        <Typography variant="caption" sx={{ display: "block" }}>
          Categories Data Exists: {categories ? "Yes" : "No"}
        </Typography>
        <Typography variant="caption" sx={{ display: "block" }}>
          Categories Array Exists: {categories?.data ? "Yes" : "No"}
        </Typography>
        <Typography variant="caption" sx={{ display: "block" }}>
          Categories Count: {categories?.data?.length || 0}
        </Typography>
        <Typography variant="caption" sx={{ display: "block" }}>
        ...
        {categories?.data && categories.data.length > 0 && (
          <Box sx={{ mt: 1 }}>
            ...
            {categories.data.map((c) => (
              <Typography key={c._id} variant="caption" sx={{ display: "block", ml: 2 }}>
                • {c.name} (ID: {c._id})
              </Typography>
            ))}
          </Box>
        ...
        {categories && (
          ...
          >
            ...
            <pre style={{ fontSize: 10, margin: 0 }}>{JSON.stringify(categories, null, 2)}</pre>
          </Box>
        )}
      </Box>
          ...
          <Grid item xs={12}>
            <Alert color="primary" severity="info" sx={{ mt: 3, fontSize: 13 }}>
              <ul style={{ margin: "0", padding: "0" }}>
                ...
                <li>Select a parent category for this subcategory</li>
              </ul>
            </Alert>
          </Grid>
        </Grid>
